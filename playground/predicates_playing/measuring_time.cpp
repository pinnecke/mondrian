...
int main() {
    ...
    mondrian::vpipes::point_copy<size_t>::func_t point_copy_func = [](size_t *out, const size_t *tupletids,
                                                                      size_t num_of_ids) {
    ...
    if (results_file.is_open()) {
        for (auto i = 0; i < 11; i += 2) {
            ...
            double sf = (1.0 * measurer.get_result_size()) / (1.0 * input_size);
            results_file << std::fixed << std::setprecision(1) << sf << "," << time_res << ","
                         << "optimizied_branch_free" << "\n";
        }
    } else {
        std::cerr << "could not open the results file" << endl;
    }
