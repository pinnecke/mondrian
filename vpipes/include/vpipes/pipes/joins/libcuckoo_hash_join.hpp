...
namespace mondrian
{
    namespace vpipes
    {
        namespace bi_pipes
        {
            ...
            class cuckoo_hash_join : public bi_pipe<Input, Input>
            {
            ...
            private:
                ...
                cuckoohash_map<output_t, vector<input_t> > join_map;

            public:
                cuckoo_hash_join (__in__ consumer_t *destination,
                ...
                {
                    return "vpipes::pipes::cuckoo_hash_join";
                }
                                ...
                                __in__ const mtl::smart_bitmask *null_mask) __attribute__((always_inline))
                {
                    ...
                    if (null_mask->is_unset()) {
                        ...
                        for (auto i = 0; i < batch_size; ++i) {
                              if (join_map.contains(data_values[i])) {
                                  join_map.find(data_values[i]).push_back(data_ids[i]);
                              } else {
                                  join_map.insert(data_values[i],vector<input_t> {data_ids[i]});
                              }
                        }
                    } else {
                                ...
                                __in__ const mtl::smart_bitmask *null_mask) __attribute__((always_inline))
                {
                    ...
                    if (null_mask->is_unset()) {
                        ...
                        for (auto i = 0; i < batch_size; ++i) {
                            ...
                            auto ids_for_data_values = join_map.find(data_values[i]);
                            auto num_ids = ids_for_data_values.size();
                            ...
                            for (auto j = 0; j < num_ids; ++j) {
                                ...
                                inner_operand_matching_indicies.set(offset + j,ids_for_data_values.at(j));
                            }
                            offset += num_ids;
                        }
                    } else {
