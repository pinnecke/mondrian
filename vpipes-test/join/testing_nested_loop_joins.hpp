 ...
 */
TEST(TestNestedLoopJoin, TestEquiJoinNoneNull)
{
    ...
    EXPECT_EQ(20, b_join.get_join_statistics()->count_join_pairs);
    EXPECT_EQ(6, b_join.get_join_statistics()->count_non_null_branch_used);
    EXPECT_EQ(0, b_join.get_join_statistics()->count_null_values);
    EXPECT_EQ(0, b_join.get_join_statistics()->count_null_branch_used);
 ...
 *      vals: it will take only one value 7 ... the ID which contains 7 in the right hand column
 */
TEST(TestNestedLoopJoin, TestEquiJoinNull)
{
    ...
    EXPECT_EQ(9, b_join.get_join_statistics()->count_join_pairs);
    EXPECT_EQ(3, b_join.get_join_statistics()->count_non_null_branch_used);
    EXPECT_EQ(11, b_join.get_join_statistics()->count_null_values);
    EXPECT_EQ(3, b_join.get_join_statistics()->count_null_branch_used);
 ...
 */
TEST(TestNestedLoopJoin, TestEquiJoinDiffLengthColumns)
{
    ...
    EXPECT_EQ(9, b_join.get_join_statistics()->count_join_pairs);
    EXPECT_EQ(4, b_join.get_join_statistics()->count_non_null_branch_used);
    EXPECT_EQ(0, b_join.get_join_statistics()->count_null_values);
    EXPECT_EQ(0, b_join.get_join_statistics()->count_null_branch_used);
    EXPECT_EQ(true, has_same_vals(expected_ids, ids_result, ids_res_length));
...
/**
 * Test: test basic join on bigger than condition between two columns of different sizes
 *       rhs: contains 20 elements values from (0, 19)
 *       lhs: contains 9 elements with only value of 5.
 *       join condition like this : lhs > rhs
 *  Right hand side operand
 *  +------------+------+------+
 *  | Tuplet id	 | val	| null |    Add to result
 *  +============+======+======+
 ...
 *
 *  The outcome should be
 *      2 batches containing
 *      ids : Ids of the lhs
 *      values : ids of the rhs that are lower than five
 */
TEST(TestNestedLoopJoin, TestBiggerThanJoinDiffLengthColumns)
{
    std::cout << "\n\n>>> Test 'TestBiggerThanJoinDiffLengthColumns'..." << std::endl;
    ...
    auto expected_ids = new size_t [ids_res_length];
    fill (expected_ids, expected_ids + 5, 0);
    fill (expected_ids + 5, expected_ids + 10, 1);
    fill (expected_ids + 10, expected_ids + 15, 2);
    fill (expected_ids + 15, expected_ids + 20, 3);

    auto expected_vals = new size_t [vals_res_length];
    std::iota (expected_vals, expected_vals + 5, 0);
    std::iota (expected_vals + 5, expected_vals + 10, 0);
    std::iota (expected_vals + 10, expected_vals + 15, 0);
    std::iota (expected_vals + 15, expected_vals + 20, 0);
